...
def get_brokul_menu_item(item_id):
    """
    Получает данные пункта меню Brokul без сжатия
    """
    session = requests.Session()
    
    # Настройка заголовков как у браузера, но без сжатия
    ...
    try:
        ...
        with open("login_page_no_compression.html", "w", encoding="utf-8") as f:
            f.write(login_page.text)
        print("   HTML сохранен в login_page_no_compression.html")
        ...
        with open("login_result_no_compression.html", "w", encoding="utf-8") as f:
            f.write(login_response.text)
        print("   Результат входа сохранен в login_result_no_compression.html")
        
        # Проверяем, есть ли ошибка входа
        if "Nieprawidłowy klucz formularza" in login_response.text:
            print("❌ Ошибка: Неправильный ключ формы")
            return {
                'success': False,
        ...
        if "customer/account/" in login_response.url or "customer/diets/" in login_response.url:
            ...
            # Сохраняем HTML
            html_filename = f"brokul_item_{item_id}_no_compression.html"
