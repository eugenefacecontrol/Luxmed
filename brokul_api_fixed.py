...
class BrokulFixedAPI:
    ...
    def safe_filename(self, url):
        ...
        path = url.replace(self.base_url, '').replace('https://', '').replace('http://', '')
        # Replace invalid characters
        safe_path = re.sub(r'[<>:"/\\|?*]', '_', path)
        if not safe_path:
            safe_path = 'root'
        return safe_path[:50]  # Limit length
    ...
    def debug_response(self, response, step_name):
        ...
        # Save response content for analysis with safe filename
        safe_name = self.safe_filename(response.url)
        filename = f"debug_{step_name.lower().replace(' ', '_')}_{safe_name}.html"
        try:
    ...
    def check_login_success(self, response):
        ...
        # Success indicators
        success_indicators = [
            "customer/account" in url and "login" not in url,
            "dashboard" in url,
            "account" in url and "login" not in url,
            "welcome" in html_content,
            "logout" in html_content,
            "my account" in html_content,
        ]
        
        # Failure indicators
        failure_indicators = [
            "customer/account/login" in url,
            "login required" in html_content,
            "invalid" in html_content,
            "incorrect" in html_content,
            "failed" in html_content,
        ]
        
        success_count = sum(1 for indicator in success_indicators if indicator)
        failure_count = sum(1 for indicator in failure_indicators if indicator)
        
        print(f"Success indicators found: {success_count}")
        print(f"Failure indicators found: {failure_count}")
    ...
    def login(self, email, password):
        ...
        try:
            ...
            login_success = self.check_login_success(login_response)
        ...
        try:
            ...
            for menu_url in menu_urls:
                try:
                    ...
                    self.debug_response(response, f"Menu Page {self.safe_filename(menu_url)}")
...
def test_brokul_fixed_api():
    """Test Brokul login using fixed API approach"""
    if not BROKUL_PASS:
        print("❌ BROKUL_PASS environment variable is not set")
        log_run("FAIL", "BROKUL_PASS not set")
        return
    
    api = BrokulFixedAPI(debug=True)
    ...
    try:
        ...
        if login_success:
            ...
            if menu_success:
                message = f"✅ Brokul Fixed API test passed! Successfully logged in and accessed menu."
                send_discord_message(message)
                log_run("PASS", "Fixed API login and menu access successful")
                print("✅ Test passed!")
            else:
                message = f"⚠️ Brokul Fixed API test partially successful - login worked but menu access failed"
                send_discord_message(message)
                log_run("PARTIAL", "Login successful but menu access failed")
                print("⚠️ Test partially successful")
        else:
            message = f"❌ Brokul Fixed API test failed - login unsuccessful"
            send_discord_message(message)
            log_run("FAIL", "Login failed")
            print("❌ Test failed")
            
    except Exception as e:
        error_msg = f"❌ Brokul Fixed API test error: {str(e)}"
        send_discord_message(error_msg)
        log_run("FAIL", str(e))
        print(f"❌ Test error: {e}")

if __name__ == "__main__":
    test_brokul_fixed_api() 